...
func windowsRemoveOneLiner() string {
	return "& { $sn='TrinityProxyAgent'; $dir=Join-Path $env:ProgramFiles 'TrinityProxy'; $pn=@('trinityproxy','TrinityProxyAgent'); $svc=Get-Service -Name $sn -EA SilentlyContinue; if($svc -and $svc.Status -eq 'Running'){Stop-Service $sn -EA SilentlyContinue; $dl=(Get-Date).AddSeconds(15); while((Get-Date) -lt $dl -and ($s=Get-Service $sn -EA SilentlyContinue) -and $s.Status -eq 'Running'){Start-Sleep 1}; if(($s=Get-Service $sn -EA SilentlyContinue) -and $s.Status -eq 'Running'){Stop-Service $sn -Force -EA SilentlyContinue; Start-Sleep 2}}; foreach($n in $pn){Get-Process -Name $n -EA SilentlyContinue | Stop-Process -Force -EA SilentlyContinue}; Start-Sleep 2; if(Get-Service -Name $sn -EA SilentlyContinue){sc.exe delete $sn | Out-Null; Start-Sleep 2}; $t=Get-ScheduledTask -TaskName $sn -EA SilentlyContinue; if($t){Stop-ScheduledTask -TaskName $sn -EA SilentlyContinue; Unregister-ScheduledTask -TaskName $sn -Confirm:$false}; Get-NetFirewallRule -DisplayName 'TrinityProxy SOCKS5*' -EA SilentlyContinue | Remove-NetFirewallRule; if(Test-Path $dir){for($i=1;$i -le 5;$i++){try{Remove-Item -LiteralPath $dir -Recurse -Force -EA Stop; break}catch{if($i -eq 5){throw}; Start-Sleep 2; foreach($n in $pn){Get-Process -Name $n -EA SilentlyContinue | Stop-Process -Force -EA SilentlyContinue}}}}; Write-Host 'TrinityProxy agent removed. Delete the node in the dashboard Agents page if needed.' }"
}
